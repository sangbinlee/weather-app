import type { RemoteSearchResultItem } from '../models/RemoteSearchResultItem';
...
const useSearchCity = () => {
  ...
  const fetchCities = () => {
    fetch(
      `https://api.openweathermap.org/geo/1.0/direct?q=${query}&limit=5&appid=75337f7452c330253bfc3e2f7f81adfd`,
    )
      .then((r) => r.json())
      .then((cities) => {
        setSearchResults(
